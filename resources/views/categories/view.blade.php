    ...
    <div class="flex gap-8 px-6 py-10">
                    ...
                    @if($cat->children->count())
                        <ul class="ml-3 mt-1 list-disc text-xs text-gray-500">
                            @foreach($cat->children as $sub)
                                <li>
                                    <a href="{{ route('category.show', $sub->slug) }}" class="hover:text-blue-500">
                                        {{ $sub->name }}
                                    </a>
                                </li>
                            @endforeach
        ...
        <main class="flex-1">
            ...
            <div class="text-sm text-gray-500 mb-2">
                <a href="/">Home</a> / {{ strtoupper($category->name) }}
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                @foreach($products as $product)
                    <a href="{{ route('product.show', $product->slug) }}" class="group relative overflow-hidden rounded shadow">
                        ...
                        <div class="absolute bottom-0 w-full bg-black/70 text-white text-center py-2">
                            <span
                                class="block font-bold text-sm tracking-wide uppercase">{{ strtoupper($product->name) }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
