...
@section('content')
                    ...
                    <!-- Left Panel -->
                        <div class="space-y-4">
                                        ...
                                        @foreach (\App\Models\Post::where('type', 'page')->get() as $page)
                                            <template x-if="'{{ strtolower($page->title) }}'.includes(searchPages.toLowerCase())">
                                                <div class="flex justify-between text-sm py-1 items-center">
                                                    ...
                                                    <button type="button"
                                                        @click="addItem({ title: '{{ $page->title }}', type: 'page', reference_id: {{ $page->id }}, url: '/{{ $page->slug }}' })"
                            ...
                            <div x-data="{ openCategories: false, searchCategories: '' }"
                                class="bg-white dark:bg-neutral-900 shadow rounded p-4">
                                <div @click="openCategories = !openCategories" class="flex justify-between items-center cursor-pointer mb-3">
                                    <h3 class="font-semibold flex items-center gap-2 cursor-pointer">
                                        💂️ Categories
                                    </h3>
                            ...
                            <div class="bg-white dark:bg-neutral-900 shadow rounded p-4 space-y-2">
                                ...
                                <input type="text" x-model="customLink.url" placeholder="URL" class="w-full border rounded px-2 py-1 text-sm" />
