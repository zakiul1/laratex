    ...
    <form method="POST" action="{{ $formAction }}" enctype="multipart/form-data"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="seo[keywords]"
                            value="{{ old('seo.keywords', $seo['keywords'] ?? '') }}" class="w-full border rounded p-2">
                    </div>
                    <div>
        ...
        <div class="space-y-6">
            ...
            <div class="border rounded p-4 bg-white shadow">
                ...
                <select name="template" class="w-full border rounded p-2">
                    ...
                    @foreach ($templates as $key => $label)
                        <option value="{{ $key }}"
                            {{ old('template', $post->template ?? '') === $key ? 'selected' : '' }}>{{ $label }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="border rounded p-4 bg-white shadow" x-data="categoryBox(window.categoriesBoxData, window.initialSelected)">
                ...
                {{-- list with conditional scroll --}}
                <div class="mb-4 space-y-1" x-bind:class="filteredList.length > 4 ? 'max-h-32 overflow-y-auto' : ''">
                ...
                <div class="mt-2">
                    <button type="button" @click="showAdd = !showAdd; errorMsg = ''" class="text-sm text-blue-600 mb-2">
                        <span x-show="!showAdd">+ Add New Category</span>
                    ...
                    <div x-show="showAdd" class="space-y-2">
                        ...
                        <button type="button" @click="addCategory()"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm px-3 py-2 rounded">
                            Add Category
                        </button>

                        <!-- Error message from AJAX -->
                        <p x-show="errorMsg" x-text="errorMsg" class="text-sm text-red-600 mt-1"></p>
                    </div>
                </div>
            ...
            <div class="border rounded p-4 bg-white shadow">
                ...
                <div class="grid grid-cols-4 gap-2 mb-4">
                    ...
                    <button type="button" @click="$data.openMedia()"
                        class="w-full h-24 flex items-center justify-center border-2 border-dashed rounded text-gray-500 hover:bg-gray-100">+</button>
                </div>
                ...
                <div class="space-y-2">
                    <template x-for="(field, idx) in fields" :key="idx">
                        <div class="flex space-x-2 items-center">
                            <input type="text" :name="`meta[${idx}][key]`" x-model="field.key" placeholder="Meta Key"
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('categoryBox', (all, selected) => ({
                ...
                get flatList() {
                    const roots = this.all.filter(c => c.parent === 0);
                    const subs = this.all.filter(c => c.parent !== 0);
                    let out = [];
                    roots.forEach(r => {
                        out.push({
                            ...r,
                ...
                async addCategory() {
                    ...
                    try {
                        ...
                        const res = await fetch('{{ route('admin.posts.categories.store') }}', {
                            method: 'POST',
                            headers: {
                        ...
                        if (!res.ok) {
                            ...
                            this.errorMsg = json.message || 'Failed to create category';
                            return;
                        }
                        const json = await res.json();
                        this.all.push(json);
