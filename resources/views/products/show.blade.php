    ...
    <section class="bg-white py-12">
        <div class="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div class="relative">
                ...
                {{-- Optional Zoom/Modal Placeholder --}}
                <div class="absolute top-3 right-3 bg-white text-black text-xs px-2 py-1 rounded shadow">ZOOM</div>
            </div>

            {{-- Product Info --}}
            <div>
                <h4 class="text-sm text-gray-500">{{ $product->category->name ?? '' }}</h4>
                        ...
                        <div class="text-center group">
                            <a href="{{ route('product.show', $item->slug) }}">
                                <img src="{{ asset('storage/' . $item->featured_image) }}" alt="{{ $item->name }}"
                                    class="w-full h-60 object-cover border border-gray-200 rounded shadow-sm group-hover:scale-105 transition duration-300">
                            </a>
